...
class Model_control_SGD(): 
    ...
    def __init__(self,raw_measurementdata):
        ...
        self.iteration = 950                                   # number of iteration for running algorithm
        ...
        self.step_startcontrol = 800                           #step to start the control
        ...
        start_step_data = 4070                            #the time step to extract subdata from whole dataset
         
        velocity_data  = 1e-5*np.array(raw_measurementdata['Stellwert_Ventil'][start_step_data+total_shift:start_step_data+iteration+total_shift])  # get control data 
        
        control_data = 1*np.array(raw_measurementdata['vel_local'][start_step_data:start_step_data+iteration])                                      # get velocity data 
        # smoothed distance 
        smoothed_dist = np.array(raw_measurementdata['smoothed_dist'][start_step_data:start_step_data+iteration])
...
# SGD optimizer for linear model
    ...
    def LinearSGD_optimizer(self,learning_rate,batchsize,local_vel,parameter_vector,current_control):
       ...
       #calculate gradient of error function
        ...
        gradient_subterm_w3 = local_vel*(w_3*local_vel +w_4 - current_control)
        gradient_subterm_w4 =  w_3*local_vel +w_4 - current_control
...
#    design_matrix = np.array([local_vel**3,local_vel**2,local_vel,1]).flatten()
 ...
 #calculate error 
    ...
    def data_fitting_model_main(self,counter,step_startcontrol,op_amplitude,op_freq, sampling_freq, op_back_vel, predicted_distance,batchsize,initial_w,iteration,learning_rate,control_data,velocity_data,smoothed_dist):
...
if  __name__ == '__main__':
    ...
    #start training with data
        R.data_fitting_model_main(counter,step_startcontrol,op_amplitude,op_freq, sampling_freq, op_back_vel, predicted_distance,batchsize,initial_w,iteration,learning_rate,control_data,velocity_data,smoothed_dist)
